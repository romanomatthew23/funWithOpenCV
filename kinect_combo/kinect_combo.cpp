...
int main( int argc, char* argv[] )
{
	...
	//int circle_flag = 1;		     // 1 to have circle detection window
	vector<vector<Point> > squares;      //for squares
		...
		*/			
		//save computational power and don't display the depth map
                }
                else
                {
            ...
            if( retrievedImageFlags[3] && capture.retrieve( bgrImage, CAP_OPENNI_BGR_IMAGE ) )  {
                //imshow( "rgb image", bgrImage );                      //save computational power
			//square detection
